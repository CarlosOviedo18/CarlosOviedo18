<!-- Import Poppins Font -->
<div style="font-family: 'Poppins', sans-serif;">
<link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600&display=swap" rel="stylesheet">
<div align="center">
  <h1 style="font-weight:600;">Carlos Oviedo Ballestero</h1>
  <p><strong>Ingeniero en Sistemas · Desarrollador Full-Stack</strong></p>
  <p>Construyo aplicaciones web reales, de la idea a producción</p>
</div>
<p align="center">
  <a href="https://www.linkedin.com/in/carlos-oviedo-135a1426b/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:caantoni2@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>
<p align="center">
  <img src="https://img.shields.io/badge/Inglés-B2-blue?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Ubicación-Costa%20Rica-success?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Estado-Disponible-brightgreen?style=for-the-badge"/>
</p>
<br/>
<table align="center" border="0">
  <tr>
    <td width="55%" style="border: none; vertical-align: top;">
      <h2>Sobre mí</h2>
      <p>
        ¡Hola! Soy Ingeniero en Sistemas de <b>Costa Rica</b> 🇨🇷 y desarrollador full-stack.
        Me enfoco en construir interfaces web limpias y funcionales, combinando un diseño visual
        atractivo con una lógica de backend sólida.
      </p>
      <p>
        Ya llevé un proyecto real de principio a fin: desarrollé y desplegué en producción una
        plataforma de e-commerce completa para un cliente, incluyendo APIs REST y una pasarela de pagos.
      </p>
      <p>
        Hoy combino ese trabajo full-stack con <b>IA aplicada</b>: sistemas RAG que responden preguntas
        sobre documentación propia, con recuperación semántica y respuestas citadas.
      </p>
    </td>
    <td width="45%" align="center" style="border: none;">
      <h2>Tecnologías</h2>
      <p><b>Frontend:</b></p>
      <img src="https://skillicons.dev/icons?i=html,css,scss,js,react,nextjs,vite"/>
      <p><b>Backend:</b></p>
      <img src="https://skillicons.dev/icons?i=nodejs,express,python,fastapi,java,php,dotnet,cs"/>
      <p><b>Base de datos:</b></p>
      <img src="https://skillicons.dev/icons?i=mysql,postgres"/>
      <p><b>IA / RAG:</b></p>
      <img src="https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white"/>
      <img src="https://img.shields.io/badge/Embeddings-FF6F00?style=for-the-badge"/>
      <img src="https://img.shields.io/badge/Vector_DB-1C1C1C?style=for-the-badge"/>
      <p><b>Nube y DevOps:</b></p>
      <img src="https://skillicons.dev/icons?i=aws,docker"/>
      <p><b>Herramientas:</b></p>
      <img src="https://skillicons.dev/icons?i=git,linux,figma,vscode,wordpress"/>
      <p><b>Análisis de Datos:</b></p>
      <img src="https://img.shields.io/badge/Power_BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black"/>
      <img src="https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white"/>
      <img src="https://img.shields.io/badge/DAX-0078D4?style=for-the-badge"/>
    </td>
  </tr>
</table>
<br/>
<!-- ═══════════════════════════════════════════════ -->
<!-- PROYECTOS DESTACADOS                            -->
<!-- ═══════════════════════════════════════════════ -->
<h2 align="center">Proyectos destacados</h2>
<!-- 1. Sircof Coffee (proyecto estrella full-stack) -->
<table align="center" border="0" width="85%">
  <tr>
    <td width="48%" style="border: none; vertical-align: middle;">
      <img width="100%" src="https://github.com/user-attachments/assets/ce87d58f-2662-4294-a68b-b30113e12de9" />
    </td>
    <td width="52%" style="border: none; vertical-align: top; padding-left: 20px;">
      <h3>Sircof Coffee – Plataforma E-commerce</h3>
      <p>Plataforma de comercio electrónico full-stack desarrollada como un proyecto real de negocio.<br/>Catálogo de productos, carrito de compras, pagos seguros con pasarela integrada y panel de administración.</p>
      <br/>
      <img src="https://skillicons.dev/icons?i=js,react,nextjs,nodejs,express,mysql,html,css,git" height="32"/>
      <br/><br/>
      <a href="https://sircof-ecommerce1.vercel.app/">
        <img src="https://img.shields.io/badge/Demo_en_vivo-00d2ff?style=for-the-badge"/>
      </a>
      &nbsp;
      <a href="https://github.com/CarlosOviedo18/sircof-ecommerce">
        <img src="https://img.shields.io/badge/Repositorio-black?style=for-the-badge&logo=github"/>
      </a>
    </td>
  </tr>
</table>
<br/>
<!-- 2. Chat RAG de Café (IA aplicada) -->
<table align="center" border="0" width="85%">
  <tr>
    <td width="52%" style="border: none; vertical-align: top; padding-right: 20px;">
      <h3>Chat RAG de Café – Asistente con IA</h3>
      <p>Asistente conversacional que responde preguntas sobre café —variedades, procesos, tueste, métodos de preparación— usando <b>RAG (Retrieval-Augmented Generation)</b> sobre un corpus propio en lugar de depender solo del modelo.</p>
      <ul>
        <li><b>Ingesta y chunking</b> de documentos, con embeddings almacenados en base vectorial.</li>
        <li><b>Búsqueda semántica</b> del contexto más relevante para cada pregunta.</li>
        <li><b>Respuestas con fuentes citadas</b>, para reducir alucinaciones y hacer verificable cada dato.</li>
        <li>API en <b>FastAPI</b> e interfaz de chat en <b>Next.js</b> con respuesta en streaming.</li>
      </ul>
      <br/>
           <img src="https://skillicons.dev/icons?i=python,fastapi,react,vite,tailwind,js" height="32"/>
      <br/>
      <img src="https://img.shields.io/badge/Claude_Haiku_4.5-D97757?style=flat-square&logo=claude&logoColor=white"/>
      <img src="https://img.shields.io/badge/Voyage_AI-6E56CF?style=flat-square"/>
      <img src="https://img.shields.io/badge/ChromaDB-FF6F00?style=flat-square"/>
      <img src="https://img.shields.io/badge/RAG-1C1C1C?style=flat-square"/>
      <br/><br/>
      <!-- 👉 Cuando despliegues el demo, descomentá este bloque y pegá la URL:
      <a href="URL_DEL_DEMO">
        <img src="https://img.shields.io/badge/Demo_en_vivo-00d2ff?style=for-the-badge"/>
      </a>
      &nbsp;
      -->
      <a href="https://github.com/CarlosOviedo18/rag-document-chat">
        <img src="https://img.shields.io/badge/Repositorio-black?style=for-the-badge&logo=github"/>
      </a>
    </td>
    <td width="48%" style="border: none; vertical-align: middle;">
      <!-- 👉 Reemplazá esta URL por el screenshot o GIF de tu chat funcionando -->
      <img width="100%" alt="Chat RAG de Café" src="https://github.com/user-attachments/assets/f5237738-dd08-481b-b5d7-79d6cde68468" />
    </td>
  </tr>
</table>
<br/>
<!-- 3. Análisis de Mercado Power BI -->
<table align="center" border="0" width="85%">
  <tr>
    <td width="48%" style="border: none; vertical-align: middle;">
      <img width="100%" alt="Dashboard Power BI" src="https://github.com/user-attachments/assets/553020c6-c2ff-4b22-991d-6fe3f12fd155" />
    </td>
    <td width="52%" style="border: none; vertical-align: top; padding-left: 20px;">
      <h3>Análisis de Mercado – Café de Costa Rica</h3>
      <p>Proyecto de inteligencia de negocios (BI) que analiza 24 tostadores de café de Costa Rica.<br/>Modelo de datos en estrella, medidas DAX y un dashboard interactivo con cumplimiento de requisitos, desempeño digital, precios y geografía.</p>
      <br/>
      <img src="https://img.shields.io/badge/Power%20BI-F2C811?style=flat-square&logo=powerbi&logoColor=black"/>
      <img src="https://img.shields.io/badge/Excel-217346?style=flat-square&logo=microsoftexcel&logoColor=white"/>
      <img src="https://img.shields.io/badge/DAX-0078D4?style=flat-square"/>
      <br/><br/>
      <a href="https://app.powerbi.com/links/s4Mzs6Sz4a?ctid=1d2e59b7-fa25-4404-8b95-d02dd4b7cb98&pbi_source=linkShare">
        <img src="https://img.shields.io/badge/Dashboard_en_vivo-F2C811?style=for-the-badge&logoColor=black"/>
      </a>
      &nbsp;
      <a href="https://github.com/CarlosOviedo18/Analisis-Mercado-PowerBI">
        <img src="https://img.shields.io/badge/Repositorio-black?style=for-the-badge&logo=github"/>
      </a>
    </td>
  </tr>
</table>
<br/>
<!-- 4. Sistema de Supermercado -->
<table align="center" border="0" width="85%">
  <tr>
    <td width="52%" style="border: none; vertical-align: top; padding-right: 20px;">
      <h3>Sistema de Supermercado</h3>
      <p>Proyecto full-stack en Java enfocado en la lógica de backend y operaciones CRUD completas.<br/>Gestión de productos, inventario y control de datos.</p>
      <br/>
      <img src="https://skillicons.dev/icons?i=java,html,css" height="32"/>
      <br/><br/>
      <a href="https://github.com/CarlosOviedo18/Supermarket-FullStack-Java">
        <img src="https://img.shields.io/badge/Repositorio-black?style=for-the-badge&logo=github"/>
      </a>
    </td>
    <td width="48%" style="border: none; vertical-align: middle;">
      <img width="100%" src="https://github.com/user-attachments/assets/9284e013-87cf-47b3-ac23-685c495b2ec6" />
    </td>
  </tr>
</table>
<br/><br/>
<!-- ═══════════════════════════════════════════════ -->
<!-- PRÁCTICAS DE FRONTEND (proyectos pequeños)      -->
<!-- ═══════════════════════════════════════════════ -->
<h3 align="center">Prácticas de Frontend</h3>
<table align="center" width="90%">
<tr>
  <td width="33%" align="center" style="padding: 12px;">
    <h4>Coffee Blog</h4>
    <img width="100%" src="https://github.com/user-attachments/assets/398dfdf4-7e68-4c39-add0-e2f74f22458c" />
    <br/><br/>
    <p>Diseño de blog limpio y responsivo, enfocado en la estructura del contenido.</p>
    <img src="https://skillicons.dev/icons?i=html,css" height="32"/>
    <br/><br/>
    <a href="https://blogdecafecarlosoviedo.netlify.app/">
      <img src="https://img.shields.io/badge/Demo-00d2ff?style=flat-square"/>
    </a>
    &nbsp;
    <a href="https://github.com/CarlosOviedo18/BlogDeCafe">
      <img src="https://img.shields.io/badge/Repo-black?style=flat-square&logo=github"/>
    </a>
  </td>
  <td width="33%" align="center" style="padding: 12px;">
    <h4>Music Festival</h4>
    <img width="100%" src="https://github.com/user-attachments/assets/d3455e42-87c1-4009-8fdb-52af874c864f" />
    <br/><br/>
    <p>Landing page interactiva con interfaz moderna y comportamiento dinámico.</p>
    <img src="https://skillicons.dev/icons?i=html,scss,js" height="32"/>
    <br/><br/>
    <a href="https://festivalmusicacarlosoviedo.netlify.app/">
      <img src="https://img.shields.io/badge/Demo-00d2ff?style=flat-square"/>
    </a>
    &nbsp;
    <a href="https://github.com/CarlosOviedo18/-Music-Festival">
      <img src="https://img.shields.io/badge/Repo-black?style=flat-square&logo=github"/>
    </a>
  </td>
  <td width="33%" align="center" style="padding: 12px;">
    <h4>WebApp JS</h4>
    <img width="100%" src="https://github.com/user-attachments/assets/705eb9c1-fb1f-4595-87a6-109acdb72347" />
    <br/><br/>
    <p>Proyecto centrado en JavaScript para practicar lógica y manipulación del DOM.</p>
    <img src="https://skillicons.dev/icons?i=html,css,js" height="32"/>
    <br/><br/>
    <a href="https://interactivewebjavascriptcarlosoviedo.netlify.app/">
      <img src="https://img.shields.io/badge/Demo-00d2ff?style=flat-square"/>
    </a>
    &nbsp;
    <a href="https://github.com/CarlosOviedo18/InteractiveWebAppJS">
      <img src="https://img.shields.io/badge/Repo-black?style=flat-square&logo=github"/>
    </a>
  </td>
</tr>
</table>
<br/>
<!-- ═══════════════════════════════════════════════ -->
<!-- ENFOQUE ACTUAL                                  -->
<!-- ═══════════════════════════════════════════════ -->
<h3 align="center">Enfoque actual</h3>
<p align="center">
  • Desarrollo full-stack<br/>
  • Inteligencia Artificial aplicada: LLMs, RAG y agentes<br/>
  • Optimización de modelos y control de costos en IA
</p>
<p align="center">
  <img src="https://img.shields.io/badge/LLMs-412991?style=flat-square&logo=openai&logoColor=white"/>
  <img src="https://img.shields.io/badge/RAG-FF6F00?style=flat-square"/>
  <img src="https://img.shields.io/badge/Agentes_IA-0078D4?style=flat-square"/>
  <img src="https://img.shields.io/badge/Optimización_y_Costos-217346?style=flat-square"/>
</p>
